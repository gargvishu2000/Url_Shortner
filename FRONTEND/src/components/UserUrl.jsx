    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: title || 'I shortened this URL with our URL shortener',
          url: url
        });
              ...
              <tr key={url._id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  <div className="text-sm">
                    <a 
                      href={`http://localhost:5002/${url.short_url}`} 
                      target="_blank" 
                ...
                <td className="px-6 py-4 text-sm font-medium">
                  <div className="flex space-x-2">
                    <button
                      onClick={() => handleCopy(`http://localhost:5002/${url.short_url}`, url._id)}
                    ...
                    <button
                      onClick={() => handleShare(`http://localhost:5002/${url.short_url}`, url.full_url)}
                      className="inline-flex items-center px-3 py-1.5 border border-transparent text-xs font-medium rounded-md shadow-sm bg-indigo-600 text-white hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-200"
                    >
                      <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
